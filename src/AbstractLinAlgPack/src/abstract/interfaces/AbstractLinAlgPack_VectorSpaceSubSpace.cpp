...
VectorSpaceSubSpace::VectorSpaceSubSpace( const space_ptr_t& space, const Range1D& rng )
{
	this->initialize(space,rng);
}

void VectorSpaceSubSpace::initialize( const space_ptr_t& space, const Range1D& rng )
{
#ifdef _DEBUG
	const index_type n = space.get() ? space->dim() : 0;
	THROW_EXCEPTION(
		space.get() && !rng.full_range() && rng.ubound() > n, std::logic_error
		,"VectorSpaceSubSpace::initialize(...): Error, "
		"rng = [" << rng.lbound() << "," << rng.ubound() << "] is not in the range "
		"[1,vec->dim()] = [1," << n << "]" );
#endif
	space_ = space;
	rng_ = space.get() && rng.full_range() ? Range1D(1,space->dim()) : rng;
}
...
void VectorSpaceSubSpace::validate_range(const Range1D& rng) const
{
	...
	THROW_EXCEPTION(
		space_.get() == NULL, std::logic_error
		,"VectorSpaceSubSpace::validate_range(rng): Error, Uninitialized" );
	THROW_EXCEPTION(
		space_.get() && !rng.full_range() && rng.ubound() > n, std::logic_error
...
bool VectorSpaceSubSpace::is_compatible(const VectorSpaceBase& another_space) const
{
	...
	if(!a_space)
		return false;
	return this->rng_ == a_space->rng_ && this->space_->is_compatible(*a_space->space_);
}
