...
public:
	 ...
	 */
	virtual const space_C_ptr_t& space_C() const = 0;
	
	///
	/** Return a matrix space object for sensitivity matrix <tt>D = -inv(C)*N</tt>.
	 */
	virtual const space_D_ptr_t& space_D() const = 0;
	 ...
	 *               <tt>this->space_D()->create_member()</tt>.
	 ...
	 * <li> <tt>C != NULL || D != NULL</tt>
	 * </ul>
	 *
	 * Postconditions:<ul>
	 * <li> [<tt>C != NULL && Gc != NULL && var_dep().size() > 0 && equ_decomp().size() > 0</tt>]
	 ...
	 * <li> [<tt>C != NULL && Gh != NULL && var_dep().size() > 0 && inequ_decomp().size() > 0</tt>]
	 ...
	 * <li> [<tt>D != NULL && Gc != NULL && var_dep().size() > 0 && var_indep().size() > 0 && equ_decomp().size() > 0</tt>]
	 ...
	 * <li> [<tt>D != NULL && Gh != NULL && var_dep().size() > 0 && var_indep().size() > 0 && inequ_decomp().size() > 0</tt>]
